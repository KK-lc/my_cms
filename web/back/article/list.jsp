...
<script>
    $(document).ready(function(){//表示document元素添加ready事件后打印hello jQuery!
        let btn = $("#search");
        console.log(btn.text());//得到文本值
    })
            ...
            <table class="table table-sm table-bordered table-hover text-center">
                ...
                <c:if test="${not empty articles}">
                    <c:forEach items="${articles}" var="article">
                        <tr>
                            <td><input type="checkbox" class="ids" value="${article.articleId}"></td>
                            <td>${article.articleTitle}</td>
                            <td>${article.articleType.typeName}</td>
                            <td>${article.articleSummary}</td>
                            <td>${article.articleState == 0?'可用':'不可用'}</td>
                            <td>
                                <a href="${path}/article?cmd=toUpdate&aId=${article.articleId}" class="btn btn-primary active" role="button">修改</a>
...
<%--                                html标签自定义属性 可以直接写属性名，html规范使用date-xx--%>
                                <button class="btn btn-primary delete" name="delete" data-aId="${article.articleId}" data-aTitle="${article.articleTitle}">
...
<script type="text/javascript">
    ...
    for (let i=0;i<delBtn.length;i++){
        delBtn[i].onclick=function () {
            console.log("点击操作")
            let title = this.getAttribute("data-aTitle");
